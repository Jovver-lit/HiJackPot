...
룰렛 칸 라벨, SPIN 버튼, 데모 제목과 테스트 안내는 Grid-Caster에서 사용한 `Art/Fonts/neodgm.ttf`를 공유한다. Builder가 `Hinted Raster`, 기본 크기 16, 문자 여백 1, advance 반올림으로 설정해 작은 글자의 픽셀 경계를 유지한다.

## 1. 전체 구조 설명

이 시스템은 룰렛을 `데이터`, `저해상도 픽셀 렌더링`, `동적 콘텐츠 배치`, `회전`, `개발 테스트`로 분리한다.
...
- `RoulettePixelWheelRenderer`는 210×210 `Texture2D`에 칸 색, 타입별 도트 패턴, 경계선, 선택 팔레트를 직접 그리고 420×420으로 Point 확대한다.
- `RouletteSegmentGraphic`은 픽셀 룰렛 위에 아이콘과 텍스트를 각 칸 중앙 각도에 맞춰 배치한다. 픽셀 렌더러가 없는 기존 프리팹에서는 메시 부채꼴 폴백도 제공한다.
...
└── RouletteRoot                       RectTransform (데모: 620 x 720)
    ...
    │   ├── DynamicSegments           RoulettePixelWheelRenderer, Wheel에 Stretch
...
데모의 420×420 Wheel은 내부적으로 210×210 텍스처를 생성해 정확히 2배 확대한다. Texture는 런타임 메모리에만 존재하고 `Filter Mode=Point`, `Mip Map=Off`, `Clamp`로 설정된다. 칸이 추가·삭제·합체되거나 weight가 변할 때만 다시 생성되며 회전 중에는 Wheel Transform만 움직인다.
...
- `Scripts/RoulettePixelWheelRenderer.cs`: 210×210 Texture2D 기반 도트 부채꼴, 타입 패턴, 픽셀 경계선, 팔레트 강조
- `Scripts/RouletteSegmentGraphic.cs`: 아이콘/텍스트 자동 배치와 기존 메시 렌더링 폴백
...
2. Scene에 Canvas와 위 Hierarchy를 만든다. Canvas의 `Pixel Perfect`를 켜고, `DynamicSegments`의 크기와 Pivot을 Wheel과 동일하게 맞춘 뒤 `RoulettePixelWheelRenderer`를 추가한다.
3. `RouletteRoot`에 `RouletteController`, `RouletteSpinController`, 개발 중에는 `RouletteTest`를 추가한다.
4. Controller에 `Wheel`, `DynamicSegments`, `Pixel Wheel Renderer`를 연결한다. `Radius Override=0`이면 DynamicSegments의 짧은 변 절반을 자동 반지름으로 사용한다.
...
8. 데모는 Grid-Caster에서 가져온 `Art/Fonts/neodgm.ttf`를 Controller의 `Label Font`와 모든 고정 UI Text에 자동 할당한다. 다른 프리팹에서도 같은 Font를 연결하면 동일한 스타일을 유지할 수 있다.
...
- `RoulettePixelWheelRenderer`의 타입별 패턴을 외부 16×16 패턴 시트로 교체하면 아티스트가 Damage, Poison, Jackpot, Hijack 재질을 직접 그릴 수 있다.
- `HighlightFX`에는 Jackpot 플래시, Hijack 오염 셰이더, 선택 글로우를 연결할 수 있다. 기본 구현은 선택 칸의 픽셀 팔레트와 라벨을 점멸한다.
