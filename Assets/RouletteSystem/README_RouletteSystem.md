...
```text
Canvas
└── RouletteRoot                       RectTransform (예: 480 x 560)
    ├── FixedFrame                    Image, 외부 픽셀아트 Sprite
    ├── Wheel                         RectTransform (예: 400 x 400, Pivot 0.5/0.5)
...
6. Frame, Pointer, CenterCap에는 원하는 외부 픽셀아트 Sprite를 넣는다. 아이콘은 각 `RouletteSegmentData.icon`에 넣는다.
7. 픽셀 Sprite Import Settings는 `Filter Mode=Point (no filter)`, `Compression=None`을 권장한다. Canvas Scaler의 Reference Resolution과 배율은 정수 배율이 나오게 잡는다. 프로젝트가 픽셀 경계를 원한다면 Quality의 불필요한 Anti Aliasing도 끈다.
